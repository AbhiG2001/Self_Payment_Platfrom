  ...
  return (
    <div className="min-h-screen bg-gray-100 p-4">
      ...
      <header className="w-full bg-white shadow p-4 rounded-lg flex justify-between items-center">
        {/* <h1 className="text-3xl font-semibold text-center text-red-600">User Account</h1> */}
        <GradientText
      ...
      <main className="mt-6">
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
          {raw.UserAccount.map((UserAccount, index) => (
            <div
              key={index}
              onClick={() => navigate(UserAccount.path)}
              className="bg-white p-4 rounded shadow text-center hover:shadow-lg cursor-pointer transition"
            >
              <img
                src={UserAccount.icon}
                alt={UserAccount.title}
                className="mx-auto h-14 mb-2 object-contain"
              />
              <p className="font-semibold text-sm sm:text-base">{UserAccount.title}</p>
            </div>
          ))}
        </div>
      </main>
